...
public class Day2 {
    ...
    void calibrationInput() {
        ...
        int sumOfIds = calculatePart1(input, 12, 13, 14);
        int totalSumOfMinimumPowerOfCubes = calculatePart2(input);

        assertThat(sumOfIds).isEqualTo(8);
        assertThat(totalSumOfMinimumPowerOfCubes).isEqualTo(2286);
    }
    ...
    void actualInput() {
        ...
        assertThat(sumOfIds).isEqualTo(2369);
        assertThat(totalSumOfMinimumPowerOfCubes).isEqualTo(66363);
    }

    private int calculatePart1(String input, int maxRedCubes, int maxGreenCubes, int maxBlueCubes) {
        List<Integer> possibleGames = new ArrayList<>();

        String[] games = input.split("\n");
        for (String game : games) {
            Game game1 = new Game(game);
            if (game1.isPossible(maxRedCubes, maxGreenCubes, maxBlueCubes)) {
                possibleGames.add(game1.getId());
            }
        }
    ...
    private int calculatePart2(String input) {
        String[] games = input.split("\n");
        int totalOverGames = 0;

        for (String gameString : games) {
            Game game = new Game(gameString);

            totalOverGames += (game.getFewestCubes("red") * game.getFewestCubes("blue") * game.getFewestCubes("green"));
        }

        return totalOverGames;
    }
